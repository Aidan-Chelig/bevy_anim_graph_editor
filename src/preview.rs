...
pub use scene::{control_preview_camera, reload_preview_scene, setup_preview_scene};
...
impl PreviewState {
    ...
    pub fn clear_asset(&mut self, status: impl Into<String>) {
    ...
    for mut player in &mut players {
        ...
        for live_clip in &state.live_clips {
            ...
            if state.live_one_shots.iter().any(|one_shot| {
                runtime::live_clip_matches_one_shot(*live_clip, *one_shot)
                    && one_shot.restart_requested
            }) {
